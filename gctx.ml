...
let with_line_width (g: gctx) (l: int) : gctx =
  { g with line_width = l }

let with_line_width_toggle (g: gctx) (b: bool) : gctx =
  { g with line_width_toggle = b }
...
(* COMMENTS: For task 6, added a check for line_width_toggle. This is what
ensures that thick lines are only drawn when the checkbox is checked, regardless
of the position of the slider. *)

let set_graphics_state (gc: gctx) : unit =
  let c = gc.color in
  Graphics.set_color (Graphics.rgb c.r c.g c.b);
  let l = gc.line_width in
  Graphics.set_line_width (if gc.line_width_toggle then l else 1)
